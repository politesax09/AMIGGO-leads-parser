...
function scraper_vh() {
    ...
    var codPresup = "-", ri = "-", rli = "-", fq600 = "-", fq150 = "-", sf = "-", basica = "-";
    ...
    for (var i = 0; i < MAX_INPUT_NUM; i++)
    {
        ...
        if ($('label.control-label').eq(i).text().toLowerCase() == "potencia")
            if ($('input').eq(i).attr('value'))
            {
                vehicle += " ";
                vehicle += $('input').eq(i).attr('value');
            }
        if ($('label.control-label').eq(i).text().toLowerCase() == "combustible")
            if ($('input').eq(i).attr('value'))
            {
                vehicle += " ";
                vehicle += $('input').eq(i).attr('value');
            }
        if ($('label.control-label').eq(i).text().toLowerCase() == "cambio")
            if ($('input').eq(i).attr('value'))
            {
                vehicle += " ";
                vehicle += $('input').eq(i).attr('value');
        ...
        // Precios
        if ($('label.control-label').eq(i).text().toLowerCase() == "tú eliges terceros ampliado + robo + incendio")
            if ($('input').eq(i).attr('value'))
                ri = $('input').eq(i).attr('value');
        if ($('label.control-label').eq(i).text().toLowerCase() == "tú eliges terceros ampliado + lunas + robo + incendio")
            if ($('input').eq(i).attr('value'))
                rli = $('input').eq(i).attr('value');
        if ($('label.control-label').eq(i).text().toLowerCase() == "terceros básico")
            if ($('input').eq(i).attr('value'))
                basica = $('input').eq(i).attr('value');
        if ($('label.control-label').eq(i).text().toLowerCase() == "tú eliges todo riesgo - franquicia 600")
            if ($('input').eq(i).attr('value'))
                fq600 = $('input').eq(i).attr('value');
        if ($('label.control-label').eq(i).text().toLowerCase() == "tú eliges todo riesgo - franquicia 150")
            if ($('input').eq(i).attr('value'))
                fq150 = $('input').eq(i).attr('value');
        if ($('label.control-label').eq(i).text().toLowerCase() == "tú eliges todo riesgo - sin franquicia")
            if ($('input').eq(i).attr('value'))
